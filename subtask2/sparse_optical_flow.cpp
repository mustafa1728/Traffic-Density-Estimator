...
int main(int argc, char** argv)
{
    ...
    String videoPath, backimgPath = "/Users/tamajit/Desktop/IIT DELHI/COP290/COP290_task1/subtask2/background.jpg";
    String userinput = "0";
...
//                           true )/2)));
        ...
        cvtColor(prvs, prvs1, COLOR_BGR2GRAY);
            ...
            goodFeaturesToTrack(prvs1, p0, 100, 0.3, 7, Mat(), 7, false, 0.04);

            // Create a mask image for drawing purposes
            Mat mask = Mat::zeros(prvs.size(), prvs.type());
        
        // Calculate optical flow
        vector<uchar> status;
        vector<float> err;
        TermCriteria criteria = TermCriteria((TermCriteria::COUNT) + (TermCriteria::EPS), 10, 0.03);
        calcOpticalFlowPyrLK(prvs1, next, p0, p1, status, err, Size(15,15), 2, criteria);
        ...
        // Display the demo
        Mat img;
        //img = mask|final_image;//add(final_image, mask, img);
        add(final_image, mask, img);
        imshow("flow", img);
      //  cout<<"hi\n";
