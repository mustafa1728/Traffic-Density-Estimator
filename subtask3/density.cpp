...
/// Dynamic density ///
    ...
    float DensityCalculator::getDynamicDensity(int frame_number, const Mat & cropped_frame, Mat frame, Mat & prev_frame, Mat & dynamicMask, Mat * prev){
       if(frame_number == 0){
           prev_frame = frame;
       }
...
/// Displaying Intermediate images, printing to console and writing to file
    ...
    void DensityCalculator::displayDensities(int frame_number, float * density_prev, float * dynamic_prev, float queue_density, float dynamic_density){
        ...
        for(int i =0; i<speed_multiplier; i++){
            cout<<(double)(speed_multiplier*(frame_number-1) + 1 + i)/fps<<", "<<avg_density<<", "<<avg_dynamic_density<<'\n';
        }  
    ...
    string DensityCalculator::writeDensities(int frame_number, float * density_prev, float * dynamic_prev, float queue_density, float dynamic_density){
        ...
        string toWrite = "Time of video, Queue Density, Dynamic Density\n";
        if(frame_number != 0){      toWrite = "";    }

        for(int i =0; i<speed_multiplier; i++){
            toWrite.append(to_string((double)(speed_multiplier*(frame_number-1) + 1 + i)/fps));
            toWrite.append(", ");
